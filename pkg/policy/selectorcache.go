...
func (sc *SelectorCache) UpdateIdentities(added, deleted cache.IdentityCache) {
	...
	for numericID := range deleted {
		if _, exists := sc.idCache[numericID]; exists {
			...
			log.WithFields(logrus.Fields{logfields.Identity: numericID}).Warningf("UpdateIdentities: Deleting a non-existing identity")
		}
	}
	for numericID, lbls := range added {
		if _, exists := sc.idCache[numericID]; exists {
			log.WithFields(logrus.Fields{logfields.Identity: numericID}).Warningf("UpdateIdentities: Adding an existing identity")
		}
